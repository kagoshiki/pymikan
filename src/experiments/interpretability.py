...
from src.experiments.fitting_reg import train_model, test_model
...
def plot_edge_functions(model: SharedMIKANEdgeWiseEmb):
    layer1 = model.layers[0]
    layer2 = model.layers[1]

    mlp1 = layer1.shared_edge_mlp
    mlp2 = layer2.shared_edge_mlp
...
def main():
    ...
    # Model
    model = SharedMIKANEdgeWiseEmb(widths=config["widths"], shared_edge_mlp_hidden_widths=config["shared_edge_mlp_hidden_widths"], embedding_dim=8, activation=F.tanh).to(device)
